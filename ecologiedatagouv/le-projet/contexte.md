# Contexte

La **donnée** est un outil clé pour piloter la transition écologique et les politiques d'aménagement du territoire de manière juste, efficace et transparente. Malgré son potentiel, le levier numérique reste sous-utilisé.

Plusieurs difficultés se posent :

* une **dispersion des données** dans un écosystème foisonnant qui s'illustre par la coexistence de plus d'une centaine de catalogues aux échelles locale, nationale, européenne et internationale ;
* un **manque de cohérence** dans la production et la mise à disposition des données territoriales ;
* un **manque de visibilité** sur les données mobilisées par des initiatives structurantes.

Ces problématiques entrainent **la perte de synergies précieuses**, alors même que **l'action collective** est indispensable pour mener à bien la **territorialisation des objectifs de transition écologique** issus entre autres des lois "Climat et Résilience" et "Energie Climat".

<figure><picture><source srcset="../.gitbook/assets/Dark mode gitbook EDG(3) (2) (1).png" media="(prefers-color-scheme: dark)"><img src="../.gitbook/assets/chiffres clés - dispersion des données (1).png" alt=""></picture><figcaption></figcaption></figure>

Le catalogue [_ecologie_.**data.gouv**._fr_](../) répond à des enjeux clés de la transition écologique et de l'aménagement du territoire grâce à :

* un accès centralisé à des [**données**](https://ecologie.data.gouv.fr/datasets) actualisées produites par des acteurs de référence ;
* une sélection d'[**indicateurs phares**](https://ecologie.data.gouv.fr/indicators) territoriaux de la transition écologique, co-construits avec des experts métiers et accessibles via une API dédiée ;
* la publication par les membres de la communauté de [**collections thématiques**](https://ecologie.data.gouv.fr/bouquets) liées aux politiques publiques : indicateurs de suivi des COP régionales, indicateurs de mobilité, urbanisme et végétalisation, etc.

<figure><img src="../.gitbook/assets/[ecologie.data.gouv.fr] Première image Newsletter 3 (1).png" alt=""><figcaption></figcaption></figure>

## Partenaires

{% hint style="warning" %}
Contenu à venir 🚧
{% endhint %}
